...
class MDPTestbed:
    def __init__(self, gamma=0.9, iterations=1000, error=1e-5, size=5, random=False, optimal=False):
        ...
        self.gridworld[0][1].move = {"NORTH": [self.gridworld[0][1].x, size - 1],
                                     "EAST": [self.gridworld[0][1].x, size - 1],
                                     "SOUTH": [self.gridworld[0][1].x, size - 1],
                                     "WEST": [self.gridworld[0][1].x, size - 1]}
        self.gridworld[0][3].reward = {"NORTH": 5.0, "EAST": 5.0, "SOUTH": 5.0, "WEST": 5.0}
        self.gridworld[0][3].move = {"NORTH": [self.gridworld[0][3].x, 2], "EAST": [self.gridworld[0][3].x, 2],
                                     "SOUTH": [self.gridworld[0][3].x, 2], "WEST": [self.gridworld[0][3].x, 2]}
                ...
                for x in range(self.size):
                    for y in range(self.size):
                        for ac in self.actions:
                            ...
                            # print("----------------")
                            values[x][y] += self.gridworld[x][y].prob[ac] * (
                            self.gridworld[x][y].reward[ac] + self.gamma * init_values[new_pos[0]][new_pos[1]])
